<template lang="pug">
.curso-main-container.pb-3
  BannerInterno
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5.mb-5
    .titulo-principal.color-acento-contenido(data-aos="fade-down-right")
      .titulo-principal__numero
        span 1
      h1 Evaluación de la seguridad digital
    .bloque-texto-g.color-primario.p-3.p-sm-4.p-md-5.mb-5(data-aos="fade-down-right")
      .bloque-texto-g__img(
        :style="{'background-image': `url(${require('@/assets/curso/tema1/imagen1.jpg')})`}"
      )
      .bloque-texto-g__texto.p-4
        p.mb-0 A medida que las organizaciones van implementando soluciones y estrategias articuladas para la gestión de la ciberseguridad en las organizaciones, se hace necesario que esta sea evaluada para determinar si está siendo efectiva y si no se están aprovechando otras vulnerabilidades, que no han sido identificadas o se estén generando nuevas brechas a la seguridad de la información.
    .row.justify-content-center
      .col-lg-8.col-md-12.mb-4.mb-lg-0(data-aos="fade-down-right")
        p Las normas como la familia ISO 27000 y <em>frameworks</em> técnicos como NIST han incorporado dentro de su metodología las fases que llevan a que las estrategias sean evaluadas de tal manera que permita identificar debilidades y a partir de estas establecer nuevos focos de la estrategia de seguridad, teniendo como premisa, que este ejercicio debe ser permanente y continuo, debido a que las amenazas crecen de manera permanente y cada vez son más complejas.
        p A continuación, se reconocen algunas técnicas que permiten realizar una evaluación de la seguridad digital en las organizaciones y que son el insumo fundamental para determinar la efectividad y eficiencia de la estrategia propuesta: 
      .col-lg-4.col-md-12(data-aos="fade-down-left")
        img(src='@/assets/curso/tema1/imagen2.png')
    separador
    #t_1_1.titulo-segundo.color-acento-contenido(data-aos="fade-down-left")
      h2 1.1 Gestión de vulnerabilidades
    .row.justify-content-center
      .col-lg-2.col-md-4.col-sm-5.col-6.mb-4.mb-lg-0(data-aos="fade-down-right")
        img(src='@/assets/curso/tema1/figura1.svg')
      .col-lg-10.col-md-12(data-aos="fade-down-left")
        p Este ejercicio de gestionar las vulnerabilidades, que pueden ser aprovechadas para convertirse en una amenaza para una organización, es un trabajo complejo dependiendo el tamaño de la organización y de la cantidad de sus activos de información (es decir cada uno de los recursos de información útil para la organización, así como los que indirectamente son necesarios para que ese dato o información pueda funcionar) tal como se observa la caracterización de dichos tipos de activos de información en la siguiente figura:
    .titulo-sexto.color-acento-contenido.mt-5(data-aos="fade")
      h5 Figura 1. 
      span Activos de información
    .row.justify-content-center
      .col-12
        .row.fondo-1.mostrar-lg.justify-content-center(data-aos="fade-down-right")
          .col-xxl-8.col-xl-10.col-lg-12.p-4(data-aos="fade-down-right")
            img(src='@/assets/curso/tema1/figura2.svg')
        .row.fondo-3.mostrar-movil.justify-content-center.p-4(data-aos="fade-down-right")
          .col-10.fondo-2.redondearpuntas.p-3.mostrar-movil
            .row.justify-content-center
              .col-md-3.col-sm-4.col-5
                img(src='@/assets/curso/tema1/fig1.svg')
              .col-12
                p.mt-3.text-center.text-white Activo de información

          .row.mt-4.mb-1.justify-content-center(data-aos="fade-down-right")
            .col-md-10.col-sm-11.col-12
              .tarjeta-avatar-b.mb-4
                .tarjeta-avatar-b__img
                  img(src='@/assets/curso/tema1/fig2.svg')
                .tarjeta.tarjeta--azul.fondo-4
                  .p-3.p-sm-4
                    p Equipo auxiliar

          .row.justify-content-center(data-aos="fade-down-right")
            .col-md-10.col-sm-11.col-12
              .tarjeta-avatar-b.mb-4
                .tarjeta-avatar-b__img
                  img(src='@/assets/curso/tema1/fig3.svg')
                .tarjeta.tarjeta--azul.fondo-4
                  .p-3.p-sm-4
                    p Infraestructura locativa

          .row.justify-content-center(data-aos="fade-down-right")
            .col-md-10.col-sm-11.col-12
              .tarjeta-avatar-b.mb-4
                .tarjeta-avatar-b__img
                  img(src='@/assets/curso/tema1/fig4.svg')
                .tarjeta.tarjeta--azul.fondo-4
                  .p-4
                    p Tecnología
              
          .row.justify-content-center(data-aos="fade-down-right")
            .col-md-10.col-sm-11.col-12
              .tarjeta-avatar-b.mb-4
                .tarjeta-avatar-b__img
                  img(src='@/assets/curso/tema1/fig5.svg')
                .tarjeta.tarjeta--azul.fondo-4
                  .p-4
                    p Servicios

          .row.justify-content-center(data-aos="fade-down-right")
            .col-md-10.col-sm-11.col-12
              .tarjeta-avatar-b.mb-4
                .tarjeta-avatar-b__img
                  img(src='@/assets/curso/tema1/fig6.svg')
                .tarjeta.tarjeta--azul.fondo-4
                  .p-4
                    p Personas

          .row.justify-content-center(data-aos="fade-down-right")
            .col-md-10.col-sm-11.col-12
              .tarjeta-avatar-b.mb-4
                .tarjeta-avatar-b__img
                  img(src='@/assets/curso/tema1/fig7.svg')
                .tarjeta.tarjeta--azul.fondo-4
                  .p-4
                    p Aplicaciones

          .row.mb-4.justify-content-center(data-aos="fade-down-right")
            .col-md-10.col-sm-11.col-12
              .tarjeta-avatar-b.mb-5
                .tarjeta-avatar-b__img
                  img(src='@/assets/curso/tema1/fig8.svg')
                .tarjeta.tarjeta--azul.fondo-4
                  .p-3.p-sm-4
                    p Datos
    .row.my-5
      .col-11
        .row.align-items-center.justify-content-center
          .col-lg-4.col-md-6.col-sm-8.col-10.sobre(data-aos="fade-down-right")
            img(src='@/assets/curso/tema1/figura3.svg')
          .col-lg-8.col-md-12(data-aos="fade-down-left")
            p.fondo-5.correr-izq.p-5.padding-y Los ejercicios para la gestión de vulnerabilidades pueden variar de acuerdo al tipo de organización y el enfoque que se le quiera dar a la gestión; pues es muy distinto realizar una gestión de vulnerabilidades en una institución financiera o crítica frente a una pequeña o mediana empresa.
    p.my-5(data-aos="fade-down-left") El procedimiento para su desarrollo ha sido basado en siete etapas principales que abordan esta gestión, entre las que se encuentran:
    .row.justify-content-center.mb-4
      .col-lg-8.col-md-12.mb-4.mb-lg-0(data-aos="fade-down-right")
        LineaTiempoD.cambiocolor
          .row.justify-content-center(numero="1" titulo="Planeación")
            .col-xl-10.mb-4.mb-md-0
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Definición de activos
                li 
                  i.fas.fa-check-circle
                  | Planeación de la evaluación
              p Etapa en donde se define el alcance del procedimiento de gestión de vulnerabilidades y se identifica cuál será el campo de evaluación o cuáles activos serán objetos de la revisión.
          
            .col-xl-2.col-lg-4.col-md-4.col-sm-5.col-6
              figure
                img(src='@/assets/curso/tema1/icono1.svg')


          .row.justify-content-center(numero="2" titulo="Descubrimiento")
            .col-xl-10.mb-4.mb-md-0
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Recopilación de información
                li 
                  i.fas.fa-check-circle
                  | Mapeo de elementos
              p Aquí se realiza la recolección de información sobre las aplicaciones, sistemas o información y de las vulnerabilidades que se encuentran presentes, a partir de ejercicios de escaneos que permita validar sus debilidades; como resultado de esta etapa, se debe de generar un mapeo de los elementos involucrados.
            .col-xl-2.col-lg-4.col-md-4.col-sm-5.col-6
              figure
                img(src='@/assets/curso/tema1/icono2.svg')          

          .row.justify-content-center(numero="3" titulo="Análisis y evaluación del riesgo")
            .col-xl-10.mb-4.mb-md-0
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Identificación de brechas
                li 
                  i.fas.fa-check-circle
                  | Determinar impacto
              p En esta etapa se establecen las brechas a la seguridad, identificando el impacto de estas sobre cada uno de los activos de información y cómo puede afectar la organización; en este ejercicio se deben de identificar y priorizar aquellas vulnerabilidades que se consideren críticas
          
            .col-xl-2.col-lg-4.col-md-4.col-sm-5.col-6
              figure
                img(src='@/assets/curso/tema1/icono3.svg')

          .row.justify-content-center(numero="4" titulo="Definición de planes de acción")
            .col-xl-10.mb-4.mb-md-0
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Diseño plan de mitigación
                li 
                  i.fas.fa-check-circle
                  | Determina controles
              p Esta etapa debe consolidar el plan para la adopción de acciones correctivas para el control de las vulnerabilidades; estas acciones se pueden tipificar en:
              span.text-bold Controles correctivos: 
              span controles aplicados directamente sobre sistemas o aplicaciones vulnerables.
              br
              br
              span.text-bold Controles compensatorios: 
              span controles aplicados indirectamente sobre los sistemas vulnerables.
          
            .col-xl-2.col-lg-4.col-md-4.col-sm-5.col-6
              figure
                img(src='@/assets/curso/tema1/icono4.svg')

      .col-lg-4.col-md-6.col-sm-7.col-8(data-aos="fade-down-left")
        img(src='@/assets/curso/tema1/imagen3.png')

    .row.justify-content-center
      .col-lg-4.col-md-6.col-sm-7.col-8.order-lg-first.order-last(data-aos="fade-down-left")
        img(src='@/assets/curso/tema1/figura7.svg')
      .col-lg-8.col-md-12.mb-4.mb-lg-0.order-lg-last.order-first(data-aos="fade-down-right")
        LineaTiempoD.cambiocolor
          .row.justify-content-center(numero="5" titulo="Ejecución de plan de acción")
            .col-xl-10.mb-4.mb-md-0
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Implementa controles y acciones
              p En esta etapa se deben de desplegar e implementar las acciones propuestas en el plan de acción, bajo los tiempos y condiciones establecidas.
          
            .col-xl-2.col-lg-4.col-md-4.col-sm-5.col-6
              figure
                img(src='@/assets/curso/tema1/icono5.svg')


          .row.justify-content-center(numero="6" titulo="Verificación de la efectividad")
            .col-xl-10.mb-4.mb-md-0
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Verifica planes y acciones
              p Consiste en realizar la evaluación de las acciones aplicadas para determinar su validez y asertividad en la reducción a las brechas de seguridad; en algunas ocasiones, se requerirá de evaluar nuevamente o aplicar otras técnicas con el fin de garantizar su efectividad.
            .col-xl-2.col-lg-4.col-md-4.col-sm-5.col-6
              figure
                img(src='@/assets/curso/tema1/icono6.svg')          

          .row.justify-content-center(numero="7" titulo="Lecciones aprendidas")
            .col-xl-10.mb-4.mb-md-0
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Revaluar y mejorar el proceso de gestión de vulnerabilidades
              p En esta última etapa, busca que el trabajo realizado sea material de insumo para la mejora del mismo proceso, a partir de los casos acertados como de los que no fueron.
          
            .col-xl-2.col-lg-4.col-md-4.col-sm-5.col-6
              figure
                img(src='@/assets/curso/tema1/icono7.svg')

    p.my-5(data-aos="fade-down-left") A partir de estas etapas anteriormente descritas, han surgido diferentes alternativas para este ejercicio, como se puede apreciar en la siguiente figura:
    .titulo-sexto.color-acento-contenido.mb-5(data-aos="fade")
      h5 Figura 2. 
      span Metodologías para la gestión de vulnerabilidades
    .row.align-items-center.justify-content-center
      .col-lg-4.col-md-7.col-sm-8.col-12(data-aos="fade-down-left")
        .row.align-items-center.justify-content-between.fondo-8.py-2
          .col-3
            img(src='@/assets/curso/tema1/figura5.svg')
          .col-9
            h5 NIST
            p.mb-0 Guía para realizar evaluaciones de riesgos SP 800-30.
      .col-lg-4.col-md-7.col-sm-8.col-12(data-aos="fade-down-left")
        .div.px-0.px-lg-2.my-4.my-lg-0
          .row.align-items-center.justify-content-between.fondo-8.py-2
            .col-3
              img(src='@/assets/curso/tema1/figura5.svg')
            .col-9
              h5 OWASP
              p.mb-0 Proyectos de seguridad de aplicaciones <i>web</i> abiertas.
      .col-lg-4.col-md-7.col-sm-8.col-12(data-aos="fade-down-left")
        .row.align-items-center.justify-content-between.fondo-8.py-2
          .col-3
            img(src='@/assets/curso/tema1/figura5.svg')
          .col-9
            h5 ISSAF
            p.mb-0 Evaluación de la seguridad de los sistemas de información.
    .row.align-items-center.justify-content-center.my-4
      .col-lg-4.col-md-7.col-sm-8.col-12(data-aos="fade-down-left")
        .row.align-items-center.justify-content-between.fondo-8.py-2
          .col-3
            img(src='@/assets/curso/tema1/figura5.svg')
          .col-9
            h5 OSSTMM
            p.mb-0 Manual de metodología de prueba de seguridad de código abierto.
      .col-lg-4.col-md-7.col-sm-8.col-12(data-aos="fade-down-left")
        .div.px-0.px-lg-2.my-4.my-lg-0
          .row.align-items-center.justify-content-between.fondo-8.py-2
            .col-3
              img(src='@/assets/curso/tema1/figura5.svg')
            .col-9
              h5 PTES
              p Estándar de ejecución de pruebas de penetración.
      .col-lg-4.col-md-7.col-sm-8.col-12(data-aos="fade-down-left")
        .row.align-items-center.justify-content-between.fondo-8.py-2
          .col-3
            img(src='@/assets/curso/tema1/figura5.svg')
          .col-9
            h5.mb-0 WASC-TC
            p.mb-0 Clasificación de amenazas del consorcio de seguridad de aplicaciones <i>web</i>.
    p.mb-5(data-aos="fade-down-left") De las cuales se profundizará en algunas de ellas:
    .row.justify-content-center
      .col-lg-4.col-md-6.col-sm-7.col-8.mb-4.mb-lg-0(data-aos="fade-down-right")
        img(src='@/assets/curso/tema1/figura6.png')
      .col-lg-8.col-md-12(data-aos="fade-down-left")
        p #[b NIST]. Este marco de trabajo para la mejora de la seguridad cibernética en infraestructuras críticas, el cual fue generado por el Gobierno de los Estados Unidos a mediados del año 2014, brinda a las organizaciones la metodología para la reducción de riesgos cibernéticos a partir de una adecuada gestión de riesgos.
        p Este marco de trabajo, propone la gestión desde una serie de funciones, las cuales se desarrollan de manera simultánea y continua como son: #[b Identificar, Proteger, Detectar, Responder y Recuperar]. Estas funciones del marco de trabajo, conlleva a desarrollar una serie de actividades basadas en categorías, encaminadas a reducir los riesgos que se pueden presentar y afectar la información de una organización. A continuación, se puede observar un resumen de estas categorías y actividades que se desarrollan para la adecuada gestión de vulnerabilidades:
    .tarjeta.fondo-9.p-4.my-5(data-aos="fade-down-right")
      SlyderA(tipo="b")
        .row.justify-content-center
          .col-10
            h4 Funciones del <em>framework</em> de NIST
            p Identificar
          .row.justify-content-center
            .col-md-2.col-sm-3.col-4.mb-4.mb-lg-0
              figure
                img(src='@/assets/curso/tema1/figura8.svg')
            .col-lg-8.mb-4.mb-md-0
              .tabla-a.color-acento-contenido.mb-5
                table.text-center
                  thead
                    tr
                      th FUNCIÓN IDENTIFICADOR ÚNICO
                      th FUNCIONES
                      th CATEGORÍA IDENTIFICADOR ÚNICO
                      th CATEGORÍAS
                  tbody.fondo-blanco
                    tr
                      <td rowspan="6">ID</td>
                      <td rowspan="6">IDENTIFICAR</td>
                      td ID.AM
                      td Gestión de activos
                    tr
                      td ID.BE
                      td Ambiente de negocios
                    tr
                      td ID.GV
                      td Gobernanza
                    tr
                      td ID.RA
                      td Evaluación de riesgos
                    tr
                      td ID.RM
                      td Estrategia de gestión de riesgos
                    tr
                      td ID.SC
                      td Gestión del riesgo de la cadena de suministro


        .row.justify-content-center
          .col-10
            h4 Funciones del <em>framework</em> de NIST
            p Proteger
          .row.justify-content-center
            .col-md-2.col-sm-3.col-4.mb-4.mb-lg-0
              figure
                img(src='@/assets/curso/tema1/figura9.svg')
            .col-lg-8.mb-4.mb-md-0
              .tabla-a.color-acento-contenido.mb-5
                table.text-center
                  thead
                    tr
                      th FUNCIÓN IDENTIFICADOR ÚNICO
                      th FUNCIONES
                      th CATEGORÍA IDENTIFICADOR ÚNICO
                      th CATEGORÍAS
                  tbody.fondo-blanco
                    tr
                      <td rowspan="6">PR</td>
                      <td rowspan="6">PROTEGER</td>
                      td PR.AC
                      td Gestión de identidad, autenticación y control de acceso
                    tr
                      td PR.AT
                      td Conciencia y capacitación
                    tr
                      td PR.DS
                      td Seguridad de datos
                    tr
                      td PR.IP
                      td Procesos y procedimientos de protección de la información
                    tr
                      td PR.MA
                      td Mantenimiento
                    tr
                      td PR.PT
                      td Tecnología de protección
          
        .row.justify-content-center
          .col-10
            h4 Funciones del <em>framework</em> de NIST
            p Detectar
          .row.justify-content-center
            .col-md-2.col-sm-3.col-4.mb-4.mb-lg-0
              figure
                img(src='@/assets/curso/tema1/figura10.svg')
            .col-lg-8.mb-4.mb-md-0
              .tabla-a.color-acento-contenido.mb-5
                table.text-center
                  thead
                    tr
                      th FUNCIÓN IDENTIFICADOR ÚNICO
                      th FUNCIONES
                      th CATEGORÍA IDENTIFICADOR ÚNICO
                      th CATEGORÍAS
                  tbody.fondo-blanco
                    tr
                      <td rowspan="3">DE</td>
                      <td rowspan="3">DETECTAR</td>
                      td DE.AE
                      td Anomalías y eventos
                    tr
                      td DE.CM
                      td Monitoreo continuo de seguridad
                    tr
                      td DE.DP
                      td Procesos de detección

        .row.justify-content-center
          .col-10
            h4 Funciones del <em>framework</em> de NIST
            p Responder
          .row.justify-content-center
            .col-md-2.col-sm-3.col-4.mb-4.mb-lg-0
              figure
                img(src='@/assets/curso/tema1/figura11.svg')
            .col-lg-8.mb-4.mb-md-0
              .tabla-a.color-acento-contenido.mb-5
                table.text-center
                  thead
                    tr
                      th FUNCIÓN IDENTIFICADOR ÚNICO
                      th FUNCIONES
                      th CATEGORÍA IDENTIFICADOR ÚNICO
                      th CATEGORÍAS
                  tbody.fondo-blanco
                    tr
                      <td rowspan="5">RS</td>
                      <td rowspan="5">RESPONDER</td>
                      td Planificación de respuesta
                      td Planificación de respuesta
                    tr
                      td Comunicaciones
                      td Comunicaciones
                    tr
                      td Análisis
                      td Análisis
                    tr
                      td Mitigación
                      td Mitigación
                    tr
                      td Mejoras
                      td Mejoras

        .row.justify-content-center
          .col-10
            h4 Funciones del <em>framework</em> de NIST
            p Recuperar
          .row.justify-content-center
            .col-md-2.col-sm-3.col-4.mb-4.mb-lg-0
              figure
                img(src='@/assets/curso/tema1/figura12.svg')
            .col-lg-8.mb-4.mb-md-0
              .tabla-a.color-acento-contenido.mb-5
                table.text-center
                  thead
                    tr
                      th FUNCIÓN IDENTIFICADOR ÚNICO
                      th FUNCIONES
                      th CATEGORÍA IDENTIFICADOR ÚNICO
                      th CATEGORÍAS
                  tbody.fondo-blanco
                    tr
                      <td rowspan="3">RC</td>
                      <td rowspan="3">RECUPERAR</td>
                      td RC.RP
                      td Planificación de la recuperación
                    tr
                      td RC.IM
                      td MEJORAS
                    tr
                      td RC.CO
                      td Comunicaciones
    .row.justify-content-center.mb-5(data-aos="fade-down-right")
      .col-lg-10.col-md-12
        .row.justify-content-center.align-items-center.fondo-10.ps-3.pe-5.py-4.redondearpuntas.mb-4
          .col-md-2.col-sm-3.col-4.p-4
            img(src='@/assets/curso/tema1/figura13.svg')
          .col-xxl-8.col-md-7.text-white
            h3.mb-0 NIST
            p.mb-4.mb-md-0 Aunque este ha sido propuesto para infraestructuras críticas, se puede aplicar a cualquier tipo de organización, lo que lo hace un método muy llamativo para entidades del gobierno. Consulte la versión completa del documento desde su sitio oficial. Este recurso también está en el material complementario.
          .col-xxl-2.col-md-3
            a.boton.color-acento-botones(:href="('https://www.nist.gov/cyberframework/framework')" target="_blank" type="application/pdf")
              span Ir al sitio <i>web</i>
              i.fas.fa-file-download
    .row.justify-content-center.mb-5
      .col-lg-7.col-md-12.mb-4.mb-lg-0(data-aos="fade-down-right")
        span.text-bold OWASP. 
        span Es una organización sin fines lucrativos, la cual busca ayudar a visibilizar las vulnerabilidades en aplicaciones para su mejoramiento de la seguridad a partir de una adecuada gestión del riesgo, siendo este un pilar fundamental para el desarrollo y aplicación de su metodología.
        br
        br
        p <i>Owasp</i> provee de un <em>framework</em> abierto el cual permite implementar auditorías en aplicaciones principalmente de tipo <i>web</i>, basados esencialmente en pruebas de caja blanca y caja negra. A partir de su ejercicio identifica las vulnerabilidades más representativas y presentes en la actualidad en su <i>Owasp</i> top 10. En su último reporte, el <i>Owasp</i> Top 10 2021 se puede apreciar la siguiente clasificación frente al informe previo del año 2017:
      .col-lg-5.col-md-6.col-sm-7.col-8(data-aos="fade-down-left")
        img(src='@/assets/curso/tema1/figura14.svg')
    .titulo-sexto.color-acento-contenido.mb-5(data-aos="fade")
      h5 Figura 3. 
      span <i>Owasp</i> Top 10 2021
    .row.justify-content-center.mb-5.fondo-11.p-2.p-sm-3.p-lg-4
      .col-lg-10.col-md-12.px-0(data-aos="fade-down-right")
        img(src='@/assets/curso/tema1/figura15.svg')
      .col-lg-10.col-md-12.fondo-12.py-2(data-aos="fade-down-left")
        p.text-small.mb-0 Nota. Tomado de <i>OWASP</i> (2021)
    p.mb-5(data-aos="fade-down-right") Con base en este reporte, se analiza el comportamiento de las vulnerabilidades identificadas, así:
    .row.justify-content-center
      .col-lg-10.col-md-12(data-aos="fade-down-right")
        AcordionA(tipo="b" clase-tarjeta="tarjeta tarjeta--gris")
          .row(titulo="Pérdida del control de acceso (<em>Broken Access Control</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p El control de acceso permite cumplir una política de permisos y roles. Estas restricciones implican que los usuarios no puedan actuar fuera de los permisos y, además, llevar un control de quien accede a cada recurso. La vulnerabilidad Broken Access Control permite que un usuario sin privilegios pueda acceder a un recurso al que no tendría que acceder. 
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Un ciberdelincuente podría actuar en el sistema con permisos de usuario o administrador.
                li 
                  i.fas.fa-check-circle
                  | Acceso a registros, directorios o archivos confidenciales para su posterior posible divulgación.              
          .row(titulo="Fallos criptográficos (<em>Cryptographic Failures</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p Hay ciertos datos que deben estar cifrados, como credenciales de acceso, datos bancarios, información confidencial de la empresa, etc., ya que aparte de que la ley lo exija, el que un ciberdelincuente se pueda hacer con ellos puede ser catastrófico para la empresa. En resumen, para que estos sean vistos únicamente por las personas autorizadas de la empresa hay que aplicarles un cifrado con algoritmos y protocolos estándares y robustos. 
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Exposición de datos sensibles a un ciberdelincuente (datos personales, críticos o estratégicos para la empresa; credenciales…).
          .row(titulo="Inyección (<em>Injection</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p Esto sucede cuando un ciberdelincuente puede enviar datos dañinos a un intérprete. Como novedad este año, el <em>Cross-site Scripting</em> forma parte de esta categoría. Para ello, hay que tener API seguras y controles de verificación a la hora de introducir los datos. 
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Exposición y posible modificación de datos sensibles por parte de un ciberdelincuente. 
                li 
                  i.fas.fa-check-circle
                  | Bajo ciertas circunstancias podría permitir al ciberdelincuente tomar el control del servidor.
          .row(titulo="Diseño inseguro (<em>Insecure Desing</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p A la hora de desarrollar una aplicación <i>web</i> es primordial incluir la seguridad de la aplicación desde la fase del diseño, ya que este año se ha incluido esta nueva categoría debido a la gran cantidad de aplicaciones que no la cumplen. Muchas aplicaciones cuentan con defectos en el diseño de las mismas. 
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Exposición y posible modificación de datos por un ciberdelincuente.
                li 
                  i.fas.fa-check-circle
                  | Acceso al servidor/aplicación por parte de un ciberdelincuente con permisos de administrador o usuario.  
          .row(titulo="Configuración de seguridad defectuosa (<em>Security Misconfiguration</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p En nuestro entorno de la aplicación <i>web</i>, los ciberdelincuentes intentarán acceder mediante cuentas por defecto, versiones obsoletas con vulnerabilidades sin actualizar, directorios desprotegidos, etc. Por ello, tiene que estar todo bien configurado y evitar usar credenciales por defecto, como puede ser en el caso de nuestro servidor, aplicaciones o dispositivos. 
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Acceso no autorizado al sistema por parte del ciberdelincuente
          .row(titulo="Componentes vulnerables y obsoletos (<em>Vulnerable and Outdated Components</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p Un ciberdelincuente podrá comprometer un sistema mediante vulnerabilidades ya conocidas en componentes comunes, como por ejemplo la versión del sistema operativo o aplicaciones instaladas en el servidor, entre otras.
              p ¿Qué impacto puede tener esto en mi empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Algunas de estas vulnerabilidades pueden tener un impacto pequeño, pero las mayores brechas de seguridad se han producido mediante la explotación de este tipo de vulnerabilidades.
          .row(titulo="Fallos de identificación y autenticación (<em>Identification and Authentication Failures</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p Esto sucede cuando en las interfaces de acceso no se controla el número de intentos de autenticación, hay una baja complejidad de las contraseñas o no se implanta un sistema multifactor “2FA”. Esto podría permitir a un ciberdelincuente realizar ataques de fuerza bruta o diccionario para ingresar en él o cuando tu aplicación permite utilizar contraseñas débiles.
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Los ciberdelincuentes tendrán acceso a cuentas administrativas o de empleados en la aplicación.
          .row(titulo="Fallos en el <em>software</em> y en la integridad de los datos (<em>Software and Data Integrity Failures</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p Muchas aplicaciones se actualizan de manera automática. Cuando estas actualizaciones no son verificadas los ciberdelincuentes podrían modificarlas cargando sus propias actualizaciones y distribuyéndolas. 
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Inclusión de código no deseado por un ciberdelincuente en mi aplicación. 
          .row(titulo="Fallos en el registro y la supervisión de la seguridad (<em>Security Logging and Monitoring Failures</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p La falta de registros sobre eventos, los denominados logs, en la aplicación o en el sistema, como inicios de sesión (tanto válidos como fallidos). Por ejemplo: que estos registros no se almacenen remotamente impide que se puedan detectar las infracciones.
              p ¿Qué impacto puede tener esto en mi empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Desconocimiento sobre inicios de sesión no autorizados.
                li 
                  i.fas.fa-check-circle
                  | Desconocimiento sobre los actos de un ciberdelincuente en nuestro sistema.
          .row(titulo="Falsificación de Solicitud del Lado del Servidor (<em>Server-side Request Forgery o SSRF</em>)")(data-aos="fade-down-right")
            .col-md-11.mb-4.mb-md-0
              p Cuando nuestra aplicación <i>web</i> obtiene un recurso externo y este no valida la URL un ciberdelincuente podría modificarla con fines malintencionados y realizar peticiones no autorizadas.
              p ¿Qué impacto puede tener esto en una empresa?
              ul.lista-ul
                li 
                  i.fas.fa-check-circle
                  | Robo de datos sensibles de la empresa.
                li 
                  i.fas.fa-check-circle
                  | Acceso a sistemas internos de la empresa
    .row.justify-content-center.mb-5
      .col-lg-10.col-md-12(data-aos="fade-down-right")
        .row.justify-content-center.align-items-center.fondo-10.ps-3.pe-5.py-4.redondearpuntas.mb-4
          .col-md-2.col-sm-3.col-4.p-4(data-aos="fade-down-right")
            img(src='@/assets/curso/tema1/figura13.svg')
          .col-xxl-8.col-md-7.text-white(data-aos="fade-down-left")
            h3.mb-0 OWASP
            p.mb-4.mb-md-0 Dado lo anterior, <i>owasp</i>, se ha vuelto una metodología práctica para la identificación de vulnerabilidades en las aplicaciones <i>web</i> más utilizadas por los equipos de desarrollo en los últimos años. Se sugiere explorar y hacer uso de esta metodología, consultando su documento oficial. Este recurso también está en el material complementario.
          .col-xxl-2.col-md-3(data-aos="fade-down-right")
            a.boton.color-acento-botones(:href="('https://www.nist.gov/cyberframework/framework')" target="_blank" type="application/pdf")
              span Ir al sitio <i>web</i>
              i.fas.fa-file-download
    .div(data-aos="fade-down-right").mb-5
      span.mb-5.text-bold ISSAF. 
      span Es un marco de trabajo para el testeo e identificación de vulnerabilidades de seguridad estructurado en tres fases. Esta metodología está enfocada en realizar análisis de seguridad a partir de los resultados obtenidos, a continuación, se observa un esquema general de sus fases y su respectiva conceptualización:
    .row.justify-content-center
      .col-12.fondo-13.pt-5
        .row.justify-content-center.mt-5(data-aos="fade-down-right")
          .col-lg-8.col-md-10.col-sm-11.col-11.fondo-gris.pt-4.px-4.redondearpuntas.borde
            .row
              .col-lg-2.col-md-2.col-sm-3.col-4
                img.subir.padd(src='@/assets/curso/tema1/nro1.svg')
            .row.justify-content-center
              .col-lg-7.col-md-12.mt-2.ps-3.ps-lg-0
                h5 Planeación y preparación
                p Establece los pasos iniciales para el desarrollo de la auditoría y ejercicios de testeo.
                p Entre las actividades relacionadas se encuentran: Identificación de interesados, reuniones de apertura, definición de enfoque y metodología, cronograma de tiempo.
              .col-lg-4.col-md-5.col-sm-6.col-7.pt-5.mb-4.mb-lg-3
                img.mostrar-lg(src='@/assets/curso/tema1/figura16.png')             

        .row.justify-content-center.my-5(data-aos="fade-down-right")
          .col-lg-8.col-md-10.col-sm-11.col-11.fondo-17.pt-4.px-4.redondearpuntas.bordeverde.my-5
            .row
              .col-lg-2.col-md-2.col-sm-3.col-4
                img.subir.padd(src='@/assets/curso/tema1/nro2.svg')
            .row.justify-content-center
              .col-lg-11.col-md-12.mt-2.ps-3.ps-lg-0
                h5 Evaluación
                p.mb-4  En esta fase, se desarrollan las validaciones de acuerdo a las siguientes 9 capas:
                img.mb-4.mostrar-lg(src='@/assets/curso/tema1/figura17.svg')
                .div.mostrar-movil.my-5
                  .row.justify-content-center
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 1. 
                      span.mb-0 Recolección de información
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 2. 
                      span.mb-0 Mapeo de la red de trabajo
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 3. 
                      span.mb-0 Identificación de vulnerabilidades
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 4. 
                      span.mb-0 Penetración
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 5. 
                      span.mb-0 Obtención de acceso y escalamiento de privilegios
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 6. 
                      span.mb-0 Enumeración
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 7. 
                      span.mb-0 Compromiso de usuarios y sitios remotos
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 8. 
                      span.mb-0 Mantener el acceso
                    .row.justify-content-center
                      .col-sm-1.col-2
                        img.sizeimg(src='@/assets/curso/tema1/flecha.svg')
                    .col-10.fondo-4.redondearpuntas.p-4
                      span.text-bold 9. 
                      span.mb-0 Cubrir rastros
        .row.justify-content-center(data-aos="fade-down-right")
          .col-lg-8.col-md-10.col-sm-11.col-11.fondo-gris.pt-4.px-4.redondearpuntas.borde.mb-5
            .row
              .col-lg-2.col-md-2.col-sm-3.col-4
                img.subir.padd(src='@/assets/curso/tema1/nro3.svg')
            .row.justify-content-center
              .col-lg-7.col-md-12.mt-2.ps-3.ps-lg-0
                h5 Reportes, limpieza y destrucción de objetos
                p Fase donde se analizan las pruebas, consolidan resultados y se presenta los reportes a los interesados.
              .col-lg-4.col-md-5.col-sm-6.col-7.mb-4.mb-lg-3
                img.mostrar-lg(src='@/assets/curso/tema1/figura18.svg')
          .row.justify-content-center.mb-5
            .col-lg-8.col-md-10.col-sm-11.col-11.fondo-12.py-2.px-4
              p.mb-0 Nota. Adaptado de OISGG (2006) 
    .row.justify-content-center.my-5
      .col-lg-10.col-md-12(data-aos="fade-down-right")
        .row.justify-content-center.align-items-center.fondo-10.ps-3.pe-5.py-4.redondearpuntas.mb-4
          .col-md-2.col-sm-3.col-4.p-4
            img(src='@/assets/curso/tema1/figura13.svg')
          .col-xxl-8.col-md-7.text-white
            h3.mb-0 ISSAF
            p.mb-4.mb-md-0 Este proyecto lleva varios años sin mantenimiento, pero se puede consultar el texto completo de la metodología en el enlace. Este recurso también puede ser consultado en el material complementario.
          .col-xxl-2.col-md-3
            a.boton.color-acento-botones(:href="('http://cuchillac.net/archivos/pre_seguridad_pymes/2_hakeo_etico/lects/metodologia_oissg.pdf')" target="_blank" type="application/pdf")
              span Ir al sitio <i>web</i>
              i.fas.fa-file-download
    .cajon-b.otrocolor.p-3.mb-5(data-aos="fade-down-right")
      span.text-bold.mb-0 OSSTMM. 
      span Esta metodología para pruebas de seguridad también es muy utilizada y, debido a su aplicación tan extensa, se ha convertido en un estándar de facto para el desarrollo de auditorías de seguridad, ya que proporciona un marco de trabajo que describe las actividades a desarrollar, las cuales están comprendidas como se presentan a continuación:
    .titulo-sexto.color-acento-contenido.mb-5(data-aos="fade")
      h5 Tabla 1. 
      span Secciones y revisiones de la metodología OSSTMM 2.1
    .row.justify-content-center.mb-5
      .col-lg-10.col-md-12(data-aos="fade-down-right")
        .tabla-a.color-acento-contenido.mb-5
          table.text-center
            caption 
              p.fuente14px.mb-0 Nota. Adaptado de ISECOM (2021) 
            thead
              tr
                th 
                  h4 SECCIÓN
                th 
                  h4 REVISIÓN
            tbody.fondo-blanco
              tr
                <td rowspan="3"><h5>Sección A -Seguridad de la Información</h5></td>
                td Revisión de la Inteligencia Competitiva
              tr
                td Revisión de Privacidad
              tr
                td Recolección de Documentos
              tr
                <td rowspan="3"><h5>Sección B - Seguridad de los Procesos</h5></td>
                td Testeo de Solicitud
              tr
                td Testeo de Sugerencia Dirigida
              tr
                td Testeo de las Personas Confiables
              tr
                <td rowspan="16"><h5>Sección C - Seguridad en las tecnologías de Internet</h5></td>
                td Identificación de los Servicios del Sistematd Logística y Controles
              tr
                td Exploración de Red
              tr
                td Identificación de los Servicios del Sistema
              tr
                td Búsqueda de Información Competitiva
              tr
                td Revisión de Privacidad
              tr
                td Obtención de Documentos
              tr
                td Búsqueda y Verificación de Vulnerabilidades
              tr
                td Testeo de Aplicaciones de Internet
              tr
                td Enrutamiento
              tr
                td Testeo de Sistemas Confiados
              tr
                td Testeo de Control de Acceso
              tr
                td Testeo de Sistema de Detección de Intrusos
              tr
                td Testeo de Medidas de Contingencia
              tr
                td Descifrado de Contraseñas
              tr
                td Testeo de Denegación de Servicios
              tr
                td Evaluación de Políticas de Seguridad
              tr
                <td rowspan="4"><h5>Sección D - Seguridad en las Comunicaciones</h5></td>
                td Testeo de PBX
              tr
                td Testeo del Correo de Voz
              tr
                td Revisión del FAX
              tr
                td Testeo del Modem
    .row.justify-content-center.my-5(data-aos="fade-down-right")
      .col-lg-10.col-md-12
        .row.justify-content-center.align-items-center.fondo-10.ps-3.pe-5.py-4.redondearpuntas.mb-4
          .col-md-2.col-sm-3.col-4.p-4
            img(src='@/assets/curso/tema1/figura13.svg')
          .col-xxl-8.col-md-7.text-white
            h3.mb-0 OSSTMM
            p.mb-4.mb-md-0 Si se desea profundizar detalles de esta metodología, se puede consultar su sitio oficial; el cual también se encuentra ubicado en el material complementario.
          .col-xxl-2.col-md-3
            a.boton.color-acento-botones(:href="('https://www.isecom.org')" target="_blank" type="application/pdf")
              span Ir al sitio <i>web</i>
              i.fas.fa-file-download
    .cajon-b.otrocolor.p-3.mb-5(data-aos="fade-down-right")
      span.text-bold.mb-0 PTES. 
      span Se consolida como estándar para pruebas de penetración y <em>testing</em>, que puede ser aplicado en cualquier organización. Entre sus objetivos se encuentra el de disponer de un marco de trabajo para la realización de auditorías técnicas de seguridad en sistemas de información. Se desarrolla en las siguientes siete fases:
    img.mb-4.mostrar-lg(src='@/assets/curso/tema1/figura19.svg')
    .row.mostrar-movil
      .col-12
        .row.justify-content-center.align-items-center.my-0.py-0(data-aos="fade-down-right")
          .col-sm-6.col-3.px-0.px-sm-5.sobre
            img(src='@/assets/curso/tema1/figura20.svg')
          .col-sm-6.col-9.correr-izq1.mt-4.mt-sm-5 
            p.padding1-y.py-2.px-5.p-sm-5.fondo-18.text-center.ms-5.ms-sm-0 Interacción previa
        .row.justify-content-center.align-items-center(data-aos="fade-down-right")
          .col-sm-6.col-3.px-0.px-sm-5.sobre.ms-3
            img(src='@/assets/curso/tema1/figura21.png')
          .col-sm-6.col-9.correr-izq2.ps-5.ps-sm-0
            p.padding2-y.fondo-18 Recopilación de información
        .row.justify-content-center.align-items-center(data-aos="fade-down-right")
          .col-sm-6.col-3.px-0.px-sm-5.sobre.ms-3
            img(src='@/assets/curso/tema1/figura22.png')
          .col-sm-6.col-9.correr-izq2.ps-5.ps-sm-0
            p.padding2-y.fondo-18 Modelado de amenazas
        .row.justify-content-center.align-items-center(data-aos="fade-down-right")
          .col-sm-6.col-3.px-0.px-sm-5.sobre.ms-3
            img(src='@/assets/curso/tema1/figura23.png')
          .col-sm-6.col-9.correr-izq2.ps-5.ps-sm-0
            p.padding2-y.fondo-18.text-center Análisis de vulnerabilidades
        .row.justify-content-center.align-items-center(data-aos="fade-down-right")
          .col-sm-6.col-3.px-0.px-sm-5.sobre.ms-3
            img(src='@/assets/curso/tema1/figura24.png')
          .col-sm-6.col-9.correr-izq2.ps-5.ps-sm-0
            p.padding3-y.fondo-18.text-center Explotación
        .row.justify-content-center.align-items-center(data-aos="fade-down-right")
          .col-sm-6.col-3.px-0.px-sm-5.sobre.ms-3
            img(src='@/assets/curso/tema1/figura25.png')
          .col-sm-6.col-9.correr-izq2.ps-5.ps-sm-0
            p.padding3-y.fondo-18.text-center Post-Explotación
        .row.justify-content-center.align-items-center(data-aos="fade-down-right")
          .col-sm-6.col-3.px-0.px-sm-5.sobre.ms-3
            img(src='@/assets/curso/tema1/figura26.png')
          .col-sm-6.col-9.correr-izq2.ps-5.ps-sm-0
            p.padding3-y.fondo-18.text-center <em>Reporting</em>
    .row.justify-content-center.my-5
      .col-lg-10.col-md-12
        .row.justify-content-center.align-items-center.fondo-10.ps-3.pe-5.py-4.redondearpuntas.mb-4
          .col-md-2.col-sm-4.col-5.p-4
            img(src='@/assets/curso/tema1/figura13.svg')
          .col-xxl-8.col-md-7.text-white
            h3.mb-0 PTES
            p.mb-4.mb-md-0 Su consulta para conocer más a detalle esta metodología puede ser a través de su sitio oficial, el cual también se encuentra en el material complementario.
          .col-xxl-2.col-md-3
            a.boton.color-acento-botones(:href="('http://www.pentest-standard.org/index.php/PTES_Technical_Guidelines')" target="_blank" type="application/pdf")
              span Ir al sitio <i>web</i>
              i.fas.fa-file-download
    p(data-aos="fade-down-right") Las metodologías anteriormente descritas permitirán realizar un análisis de vulnerabilidades, establecer planes de acciones y un proceso importante, que es adoptar planes de mejoramiento al interior, dado que el proceso de gestión de vulnerabilidades debe ser un proceso continuo.
    separador
    #t_1_2.titulo-segundo.color-acento-contenido(data-aos="fade-down-left")
      h2 1.2 Tipos de pruebas
    .row.justify-content-center.align-items-center.mb-5
      .col-lg-2.col-md-3.col-sm-4.col-5.mb-4.mb-lg-0(data-aos="fade-down-right")
        img(src='@/assets/curso/tema1/figura27.svg')
      .col-lg-10.col-md-9.col-sm-12(data-aos="fade-down-left")
        p Cuando se está desarrollando el proceso de evaluación de la seguridad, se debe de garantizar que los resultados presentados estén respaldados en ejercicios de verificación y comprobación que permitan establecer si una condición se está cumplimento o no; para este caso estamos hablando de controles, técnicas, tácticas para el aseguramiento de la información y en este ejercicio pueden surgir diferentes alternativas para verificar este tipo de cumplimientos, entre las que se pueden apreciar:
    SlyderF(data-aos="fade-down-right").mb-5(columnas="col-lg-6 col-xl-4")
      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura28.svg')
          .col-8
            h4 Pruebas unitarias
        p Este tipo de pruebas se enfocan en testear fragmentos de código o aplicaciones para evaluar la lógica bajo la cual se ha programado y si desarrolla adecuadamente las tareas bajo ciertas condiciones.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura29.svg')
          .col-8
            h4 Prueba de integración 
        p Estas pruebas, buscan verificar el desarrollo de una actividad que involucra más de un componente, por ejemplo, la lógica de una aplicación <i>web</i> y la de un motor de base de datos.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura30.svg')
          .col-8
            h4  Pruebas de extremo a extremo
        p Estas buscan verificar el traslado de información y cómo viajan entre un punto de origen y uno de destino, este tipo de pruebas son comunes cuando se prueban API’s o plataformas de envío de mensajes.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura31.svg')
          .col-8
            h4  Prueba de aceptación
        p Este tipo de pruebas, busca validar si una solución cumple con lo requerido o solicitado por un cliente.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura32.svg')
          .col-8
            h4  Pruebas de caja blanca
        p Este tipo de pruebas validan el comportamiento de la lógica de la solución, para realizar este tipo de pruebas, se asume que conoce los detalles de su arquitectura, métodos, funciones entre otros.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura33.svg')
          .col-8
            h4 Pruebas de caja negra
        p Este tipo de pruebas son de funcionalidad y comportamiento de un sistema, por lo general desconociendo la estructura de la solución.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura34.svg')
          .col-8
            h4  Pruebas de caja gris
        p Propone una validación, combinando las pruebas de caja blanca y caja negra, permitiendo abordar aspectos más complejos e integrales en las soluciones.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura35.svg')
          .col-8
            h4  Prueba manual
        p Son pruebas que se especifican manualmente y sin automatización, para la evaluación de los resultados obtenidos.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura36.svg')
          .col-8
            h4 Prueba estática
        p Son aquellas que se realizan sin código real, para determinar si hay defectos en el rendimiento de la solución.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura37.svg')
          .col-8
            h4 Pruebas dinámicas
        p Este tipo de pruebas se ejecuta con código real para validar su funcionamiento.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura38.svg')
          .col-8
            h4 Pruebas visuales o de interfaz de usuario
        p Son las pruebas que se realizan a interfaces de usuario para la revisión del comportamiento y la integridad de la información 

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura39.svg')
          .col-8
            h4 Pruebas de humo
        p Tipo de prueba que se realiza a un conjunto pequeño de controles para verificar su funcionalidad

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura40.svg')
          .col-8
            h4 Pruebas de regresión
        p Permite validar si una funcionalidad o característica se rompe o no está funcionando como se propuso.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura41.svg')
          .col-8
            h4 Pruebas de carga
        p Este tipo de pruebas buscan verificar el comportamiento de una solución de acuerdo a diferentes momentos de demanda del servicio.

      .tarjeta.sinredondeo.ajustesize.fondo-19.p-4.m-auto
        .row.justify-content-center.mb-3.align-items-center
          .col-4
            img(src='@/assets/curso/tema1/figura42.svg')
          .col-8
            h4 Pruebas de inserción
        p Este tipo de pruebas permite verificar la integridad de la seguridad de la información dentro de la solución.
    .row.fondo-20.align-items-center
      .col-lg-8.col-md-12.mb-lg-2.mb-lg-4.p-lg-5.p-3(data-aos="fade-down-right")
        p.mt-0.mt-lg-3.mt-xl-0 Existen varios tipos de pruebas que se pueden realizar a las aplicaciones o soluciones con las que cuenta la organización que permite verificar si realmente están realizando las operaciones según lo solicitado y lo establecido en el programa.
        p Su aplicación dependerá del alcance de la evaluación a realizar, así como también dependerá si las soluciones son construidas <i>in-house</i> o son adquiridas a un tercero, en cualquiera de los casos, debería de validarse la información para garantizar su integridad
      .col-lg-4.col-md-6.col-sm-7.col-8.pe-0.pb-0.my-5.mostrar-lg(data-aos="fade-down-left")
        img.subir(src='@/assets/curso/tema1/imagen4.png')
    separador
    #t_1_3.titulo-segundo.color-acento-contenido(data-aos="fade-down-left")
      h2 1.3 <em>Hacking</em> ético
    .row.justify-content-center.mb-5
      .col-lg-5.col-md-6.col-sm-7.col-8.mb-4.mb-lg-0(data-aos="fade-down-right")
        img(src='@/assets/curso/tema1/imagen5.png')
      .col-lg-7.col-md-12(data-aos="fade-down-left")
        p El <i>hacking</i> ético o también denominado sombrero blanco, es un conjunto de técnicas que se aplican para el descubrimiento de vulnerabilidades dentro de un entorno o un conjunto de aplicaciones.
        p Esta técnica se desarrolla de manera consentida y aprobada por las organizaciones y los responsables de realizar las validaciones correspondientes, con lo cual buscan identificar las debilidades, anormalidades y falencias en su seguridad.
        p Los expertos que realizan este tipo de validaciones hacen uso de técnicas especializadas que se denominan <em>Test</em> de Penetración o <em>Pentesting</em>, y su objetivo es burlar los controles de seguridad y en caso de conseguir una identificación positiva, este deberá reportarlo a la organización. Este tipo de estrategia no debería de causar daño, dado que es concebido y tiene fines de exploración.
    p.my-5(data-aos="fade-down-right") Este ejercicio se desarrolla básicamente en las siguientes 5 fases:
    .titulo-sexto.color-acento-contenido.mt-5(data-aos="fade")
      h5 Figura 4. 
      span Fases del <em>hacking</em> ético
    .row.justify-content-center.fondo-21
      .col-12
        SlyderF(data-aos="fade-down-right").my-5(columnas="col-lg-6 col-xl-4")
          .tarjeta.ajustesize.fondo-22.p-4.m-auto
            .row.mb-3
              .col-4
                img(src='@/assets/curso/tema1/figura43.svg')
            h4 Fase 1 - Reconocimiento
            p Preparación del escenario, hace uso de diferentes técnicas para el descubrimiento de pistas que le permitan alcanzar su objetivo.
            ul.lista-ul
              li.mb-0
                i.lista-ul__vineta
                p Ingeniería social
              li.mb-0
                i.lista-ul__vineta
                p <i>Dumpster diving</i>
              li.mb-0
                i.lista-ul__vineta
                p Footprinters
          .tarjeta.ajustesize.fondo-22.p-4.m-auto
            .row.mb-3
              .col-4
                img(src='@/assets/curso/tema1/figura44.svg')
            h4 Fase 2 - Escaneo
            p A partir de la información recolectada, inicia con el escaneo con la ayuda de herramientas para la búsqueda de vulnerabilidades.
            ul.lista-ul
              li.mb-0
                i.lista-ul__vineta
                | Escáner de puertos
              li.mb-0
                i.lista-ul__vineta
                | Escáner de vulnerabilidades
          .tarjeta.ajustesize.fondo-22.p-4.m-auto
            .row.mb-3
              .col-4
                img(src='@/assets/curso/tema1/figura45.svg')
            h4 Fase 3 - Obtener acceso
            p Fase en la cual explota las vulnerabilidades encontradas.
            ul.lista-ul
              li.mb-0
                i.lista-ul__vineta
                | Inyección de código
              li.mb-0
                i.lista-ul__vineta
                | Basado en <i>web</i>
              li.mb-0
                i.lista-ul__vineta
                | Basado en red
              li.mb-0
                i.lista-ul__vineta
                | Ingeniería social
              li.mb-0
                i.lista-ul__vineta
                | Ataque de contraseñas
              li.mb-0
                i.lista-ul__vineta
                | <i>Network spoofung</i>
              li.mb-0
                i.lista-ul__vineta
                | <i>Network sniffers</i>
          .tarjeta.ajustesize.fondo-22.p-4.m-auto
            .row.mb-3
              .col-4
                img(src='@/assets/curso/tema1/figura46.svg')
            h4 Fase 4 - Mantener acceso
            p Fase en donde mantiene el sistema vulnerable.
            ul.lista-ul
              li.mb-0
                i.lista-ul__vineta
                | <i>Back Doors</i>
              li.mb-0
                i.lista-ul__vineta
                | Troyanos
              li.mb-0
                i.lista-ul__vineta
                | <i>Rootkit</i>
              li.mb-0
                i.lista-ul__vineta
                | <i>Keyloggers</i>
              li.mb-0
                i.lista-ul__vineta
                | <i>Botnets</i>
          .tarjeta.ajustesize.fondo-22.p-4.m-auto
            .row.mb-3
              .col-4
                img(src='@/assets/curso/tema1/figura47.svg')
            h4 Fase 5 - Cubrir huella
            p Fase en la cual cubre todas las pistas de las acciones realizadas durante el ejercicio.
            ul.lista-ul
              li.mb-0 
                i.lista-ul__vineta
                | Troyanos
              li.mb-0
                i.lista-ul__vineta
                | <i>Rootkit</i>
              li.mb-0
                i.lista-ul__vineta
                | Esteganografía
              li.mb-0
                i.lista-ul__vineta
                | <i>Tuneling</i>         
    p.my-5(data-aos="fade-down-right") Ahora bien, dependiendo de su alcance, se puede determinar el tipo de <em>hacker</em> requerido, tal como se evidencia:
    figure(data-aos="fade-up-left")
      .video
        iframe(width="560" height="315" src="https://www.youtube.com/embed/Puk3JZ5R_Ic" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)
    .row.mostrar-lg.my-5(data-aos="fade-down-right")
      .col-12
        img(src='@/assets/curso/tema1/figura48.svg')
    .row.mostrar-movil.my-5.justify-content-center
      .col-md-6.col-sm-8.col-8
        img.mb-4(src='@/assets/curso/tema1/figura49.svg')
        img(src='@/assets/curso/tema1/figura50.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas
        p.mb-0.py-3.px-4 Enemigo del <em>Black Hat</em>, este destruye lo que este construye.
      .col-md-6.col-sm-8.col-8.mt-4
        img(src='@/assets/curso/tema1/figura51.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas.p-2
        p.mb-0.py-3.px-4 <em>Hacker</em> bueno que realiza tareas de descubrimiento de vulnerabilidades.
      .col-md-6.col-sm-8.col-8.mt-4
        img(src='@/assets/curso/tema1/figura52.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas.p-2
        p.mb-0.py-3.px-4 Realiza acciones entre buenas y malas.
      .col-md-6.col-sm-8.col-8.mt-4
        img(src='@/assets/curso/tema1/figura53.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas.p-2
        p.mb-0.py-3.px-4 <em>Hacker</em> con intenciones de hacer daño.
      .col-md-6.col-sm-8.col-8.mt-4
        img(src='@/assets/curso/tema1/figura54.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas.p-2
        p.mb-0.py-3.px-4 Persona que informa o entrega información a terceros como la competencia, por ejemplo.
      .col-md-6.col-sm-8.col-8.mt-4
        img(src='@/assets/curso/tema1/figura55.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas.p-2
        p.mb-0.py-3.px-4 <em>Hacker</em> que buscan errores en soluciones <em>software</em> para su mejora.
      .col-md-6.col-sm-8.col-8.mt-4
        img(src='@/assets/curso/tema1/figura56.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas.p-2
        p.mb-0.py-3.px-4 Dedicado al <em>hackeo</em> de cuentas de redes sociales.
      .col-md-6.col-sm-8.col-8.mt-4
        img(src='@/assets/curso/tema1/figura57.svg')
      .col-md-7.col-sm-8.col-10.fondo-23.redondearpuntas.p-2
        p.mb-0.py-3.px-4 Novatos en el ejercicio del <em>hacking</em>.
    p.my-5(data-aos="fade-down-right") Estos ejercicios son muy comunes cuando las organizaciones quieren identificar vulnerabilidades presentes en sus sistemas, y requieren que una persona externa desarrolle esta actividad, pero siempre mediando algún acuerdo de confidencialidad y naturalmente una autorización de por medio.
    separador
    #t_1_4.titulo-segundo.color-acento-contenido(data-aos="fade-down-left")
      h2 1.4  Equipos de seguridad <em>Red Team & Blue Team</em>
    .row.justify-content-center.my-5
      .col-lg-8.col-md-12.mb-4.mb-lg-0(data-aos="fade-down-right")
        p Los equipos de seguridad <em>Red Team & Blue Team</em> se consolidan como estrategias para organizaciones más complejas y de mayor tamaño y como su nombre lo indica, está conformado por equipos de personas quienes, desde cada uno de sus grupos, realizan acciones para la identificación de vulnerabilidades.
        p Este concepto proviene de elementos relacionados con entrenamiento militar, los cuales fueron adoptados por la disciplina de la ciberseguridad y que incorporaron para establecer estrategias de auditoría desde dos frentes distintos. Veamos:
      .col-md-4.col-sm-7.col-8(data-aos="fade-down-left")
        img(src='@/assets/curso/tema1/figura58.svg')
    .row
      .col-auto.fondo-24.py-3.px-5(data-aos="fade-down-right")
        h3.mb-0.text-white.ms-4 Comparación entre el <em>Red Team</em> y el <em>Blue Team</em>
    .row.justify-content-between.my-5(data-aos="fade-down-right")
      .col-lg-6.col-md-12.mb-4.mb-lg-0.fondo-25.redondearpuntas.pt-0.separar(data-aos="fade-down-right")
        .row.justify-content-center.mt-0
          .col-4.pt-0.subir1
            img.mt-0(src='@/assets/curso/tema1/figura59.svg')
          .col-8.pt-4
            h5 <em>Red Team</em>
            p Busca burlar la seguridad de la organización, y simular ataques para la identificación de vulnerabilidades, haciendo uso de técnicas de <em>pentesting</em>, herramientas especializadas e incluso de técnicas de recolección de información convencionales, todo esto realizándose desde el exterior de la organización; lo anterior teniendo en cuenta:
        .row.justify-content-center
          .col-md-6.col-sm-12.mb-4.mb-lg-0.ps-4(data-aos="fade-down-right")
            ul.lista-ul
              li 
                i.fas.fa-check-circle
                | Seguridad ofensiva
              li 
                i.fas.fa-check-circle
                | <em>Hackeo</em> ético
              li 
                i.fas.fa-check-circle
                | Explotación de vulnerabilidades
              li 
                i.fas.fa-check-circle
                | Pruebas de penetración
              li 
                i.fas.fa-check-circle
                | Pruebas de caja negra
              li 
                i.fas.fa-check-circle
                | Ingeniería social
              li 
                i.fas.fa-check-circle
                | Escaneo de aplicaciones <i>web</i> 
          .col-md-6.col-sm-7.col-8(data-aos="fade-down-left")
            img(src='@/assets/curso/tema1/figura60.svg')
      .col-lg-6.col-md-12.fondo-26.redondearpuntas.separar.pt-0(data-aos="fade-down-left")
        .row.justify-content-center.mt-0
          .col-4.pt-0.subir1
            img.mt-0(src='@/assets/curso/tema1/figura61.svg')
          .col-8.pt-4
            h5 <em>Blue Team</em>
            p Hacen frente a los ataques y defienden a la organización de los posibles riesgos de seguridad de la información, convirtiéndose en una primera línea frente a la atención a cualquier incidente de seguridad; teniendo en cuenta:
        .row.justify-content-center.mt-4
          .col-md-6.col-sm-12.mb-4.mb-lg-0.ps-4(data-aos="fade-down-right")
            ul.lista-ul
              li 
                i.fas.fa-check-circle
                | Seguridad defensiva
              li 
                i.fas.fa-check-circle
                | Protección de infraestructura
              li 
                i.fas.fa-check-circle
                | Control de daños
              li 
                i.fas.fa-check-circle
                | Respuesta al incidente
              li 
                i.fas.fa-check-circle
                | Seguridad operacional
              li 
                i.fas.fa-check-circle
                | Caza de amenazas
              li 
                i.fas.fa-check-circle
                | Forense digital
          .col-md-6.col-sm-7.col-8(data-aos="fade-down-left")
            img(src='@/assets/curso/tema1/figura62.svg')
    p.my-5(data-aos="fade-down-right") Entre algunas actividades que desarrollan los equipos se encuentran:
    AcordionA(data-aos="fade-down-right").mb-5(tipo="a" clase-tarjeta="tarjeta tarjeta--gris")
      .row.px-4(titulo="<em>Red Team</em>")
        .col-md-7.mb-4.mb-md-0.px-5
          p Entre sus actividades se encuentran:
          ul.lista-ul
            li 
              i.fas.fa-check-circle
              | Pruebas de penetración en las que un miembro del equipo rojo intenta acceder al sistema utilizando una variedad de técnicas del mundo real.
            li 
              i.fas.fa-check-circle
              | Tácticas de ingeniería social, que tienen como objetivo manipular a los empleados u otros miembros de la red para que compartan, divulguen o creen credenciales de red.
            li 
              i.fas.fa-check-circle
              | Interceptar la comunicación para mapear la red u obtener más información sobre el entorno para eludir las técnicas de seguridad más comunes.
            li 
              i.fas.fa-check-circle
              | Clonación de tarjetas de acceso de un administrador para obtener acceso a áreas sin restricciones.
    
        .col-md-5
          figure
            img(src='@/assets/curso/tema1/imagen6.png')

      .row.px-4(titulo="<em>Blue Team</em>")
        .col-md-7.mb-4.mb-md-0.px-5
          p El trabajo desarrollado por este equipo azul está enfocado en la prevención, detección y remediación.  Incorporan las siguientes capacidades:
          ul.lista-ul
            li 
              i.fas.fa-check-circle
              | Comprensión completa de la estrategia de seguridad de la organización.
            li 
              i.fas.fa-check-circle
              | Habilidades de análisis para identificar con precisión las amenazas más peligrosas y priorizar las respuestas en consecuencia.
            li 
              i.fas.fa-check-circle
              p Técnicas de endurecimiento para reducir la superficie de ataque, particularmente en lo que se refiere al sistema de nombres de dominio (DNS) para prevenir ataques de #[em phishing] y otras técnicas de violación basadas en la #[em web].
            li 
              i.fas.fa-check-circle
              | Gran conocimiento de las herramientas y sistemas de detección de seguridad existentes de la empresa y sus mecanismos de alerta.
      
        .col-md-5
          figure
            img(src='@/assets/curso/tema1/imagen7.png')
    p.my-5(data-aos="fade-down-right") Sumado a lo anterior, contar con estos equipos en las organizaciones puede fortalecer a la empresa a través de estos beneficios:
    SlyderF(data-aos="fade-down-right").my-5(columnas="col-lg-6 col-xl-4")
      .tarjeta.ajustesize1.fondo-27.p-4.m-auto
        .row.mb-3.justify-content-center
          .col-lg-7.col-md-6.col-sm-7.col-7
            img(src='@/assets/curso/tema1/figura63.svg')
        p.text-white.text-bold Identificación de configuraciones erróneas y brechas a la seguridad.

      .tarjeta.ajustesize1.fondo-27.p-4.m-auto
        .row.mb-3.justify-content-center
          .col-lg-7.col-md-6.col-sm-7.col-7
            img(src='@/assets/curso/tema1/figura64.svg')
        p.text-white.text-bold Fortalecimiento de la seguridad de la red para detectar ataques dirigidos y mejorar el tiempo de ruptura.

      .tarjeta.ajustesize1.fondo-27.p-4.m-auto
        .row.mb-3.justify-content-center
          .col-lg-7.col-md-6.col-sm-7.col-7
            img(src='@/assets/curso/tema1/figura65.svg')
        p.text-white.text-bold Aumentar la competencia sana entre el personal de seguridad y fomentar la cooperación entre los equipos de TI y seguridad.

      .tarjeta.ajustesize1.fondo-27.p-4.m-auto
        .row.mb-3.justify-content-center
          .col-lg-7.col-md-6.col-sm-7.col-7
            img(src='@/assets/curso/tema1/figura66.svg')
        p.text-white.text-bold Aumentar la conciencia entre el personal sobre el riesgo de vulnerabilidades humanas que pueden comprometer la seguridad de la organización.

      .tarjeta.ajustesize1.fondo-27.p-4.m-auto
        .row.mb-3.justify-content-center
          .col-lg-7.col-md-6.col-sm-7.col-7
            img(src='@/assets/curso/tema1/figura67.svg')
        p.text-white.text-bold Desarrollar las habilidades y la madurez de las capacidades de seguridad de la organización dentro de un entorno de capacitación seguro y de bajo riesgo.
    .row.justify-content-center.fondo-28.align-items-center.px-5
      .col-lg-3.col-md-4.col-sm-5.col-6.mb-4.mb-lg-0(data-aos="fade-down-right")
        img(src='@/assets/curso/tema1/figura68.svg')
      .col-lg-9.col-md-12(data-aos="fade-down-left")
        p Estas son algunas de las estrategias para la evaluación de la seguridad de la información en las organizaciones. Su elección dependerá del tipo, complejidad, tamaño y alcance de la evaluación que se quiera realizar.
          
</template>

<script>
export default {
  name: 'Tema1',
  components: {},
  data: () => ({
    // variables de vue
  }),
  mounted() {
    this.$nextTick(() => {
      this.$aosRefresh()
    })
  },
  updated() {
    this.$aosRefresh()
  },
}
</script>

<style lang="sass"></style>
